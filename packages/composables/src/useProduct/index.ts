...
import { ConfiguredProduct, ConfigureProductParams, GetProductParams, GetRelatedProductsParams, Product, ProductSearchParams, ProductSearchResult } from 'packages/api-client/src';
...
const isGetRelatedProductsParams = (params: any): params is GetRelatedProductsParams => (params as GetRelatedProductsParams).catId !== undefined;
const isGetProductParams = (params: any): params is GetProductParams => (params as GetProductParams).id !== undefined;
const isConfigureProductParams = (params: any): params is ConfigureProductParams => (params as ConfigureProductParams).attributes !== undefined;

const params: UseProductFactoryParams<Product | Product[] | ProductSearchResult, any> = {
  productsSearch: async (context: Context, params: GetProductParams | GetRelatedProductsParams | ProductSearchParams | ConfigureProductParams): Promise<Product | Product[] | ProductSearchResult> => {
    const { customQuery, ...searchParams } = params;

    if (isGetRelatedProductsParams(params)) return (await context.$kibo.api.getRelatedProducts(searchParams as GetRelatedProductsParams)).data?.products?.items || [];

    if (isConfigureProductParams(params)) {
      const product = (await context.$kibo.api.getProduct(searchParams as GetProductParams, customQuery)).data?.product || {} as any;
      const configuredProduct = (await context.$kibo.api.configureProduct({
        product,
    ...
    if (isGetProductParams(params)) return (await context.$kibo.api.getProduct(searchParams as GetProductParams, customQuery)).data?.product || {} as any;

    return (await context.$kibo.api.searchProducts(searchParams as ProductSearchParams)).data?.products || {} as any;
  }
};

export default useProductFactory<Product | Product[] | ProductSearchResult, any>(params);
