...
import { PaymentMethodResponse, CreditCardFormData, PaymentConfig, TokenizedCCData } from '../types';

export const getAvailablePaymentMethods = (): PaymentMethodResponse[] => {
...
export const getData = async (creditCardFormData: CreditCardFormData, paymentConfig: PaymentConfig): Promise<TokenizedCCData> => {
  try {
...
const factoryParams: UsePaymentProviderParams<any, any> = {
  ...
  save: async (context: Context, { paymentMethod }) => {
    ...
    await context.$kibo.api.addPaymentMethodToCheckout(
      paymentMethodParams
    );
  }
};
