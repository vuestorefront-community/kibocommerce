...
export const userBillingGetters: UserBillingGetters<any, any> = {
  getAddresses: (billing, criteria?: AddressCriteriaParams) => {
  ...
  getDefault: (billing) =>
    billing.find(
      (address) => address.types.find((t) => t.name === 'Billing').isPrimary
    ),
  ...
  isDefault: (address) =>
    address ? address.types.find((t) => t.name === 'Billing').isPrimary : false
};
