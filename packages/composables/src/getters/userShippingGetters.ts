...
import { AddressCriteriaParams } from '../types';
export const userShippingGetters: UserShippingGetters<any, any> = {
  getAddresses: (shipping, criteria?: AddressCriteriaParams) => {
    if (!criteria || !Object.keys(criteria).length) {
      return shipping;
    }
    const entries = Object.entries(criteria);
    return shipping.filter((address) =>
      entries.every(([key, value]) => address[key] === Number(value))
    );
  },
  getDefault: (shipping) =>
    shipping.find(
      (address) => address.types.find((t) => t.name === 'Shipping').isPrimary
    ),
  ...
  isDefault: (address) =>
    address ? address.types.find((t) => t.name === 'Shipping').isPrimary : false
};
